...
calc_deca = gp.GPAW(mode='pw',xc='LDA',txt='pw_LDA/pw_LDA_deca.txt')
calc_tree = gp.GPAW(mode='pw',xc='LDA',txt='pw_LDA/pw_LDA_tree.txt')

deca.set_calculator(calc_deca)
tree.set_calculator(calc_tree)

# Optimizing
opt_deca = BFGS(deca,logfile='pw_LDA/bfgs_deca.log')
opt_tree = BFGS(tree,logfile='pw_LDA/bfgs_tree.log')
