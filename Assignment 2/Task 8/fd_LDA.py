...
calc_deca = gp.GPAW(mode='fd',xc='LDA',txt='fd_LDA/fd_LDA_deca.txt')
calc_tree = gp.GPAW(mode='fd',xc='LDA',txt='fd_LDA/fd_LDA_tree.txt')

deca.set_calculator(calc_deca)
tree.set_calculator(calc_tree)

# Optimizing
opt_deca = BFGS(deca,logfile='fd_LDA/bfgs_deca.log')
opt_tree = BFGS(tree,logfile='fd_LDA/bfgs_tree.log')
